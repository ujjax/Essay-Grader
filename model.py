...
class Config(object):
    ...
    maxlen = 750  # Sequence longer then this get ignored
    batch_size = 200  # The batch size during training.
    dataset = 'data_gen'  # Parameter for extra option
    noise_std = 0.
...
def main(unused_args):
    ...
    print('Loading data')
    train, valid = data_gen.load_data(n_words=n_words, valid_portion=0.1, maxlen=maxlen)

    train = data_gen.prepare_data(train[0], train[1], maxlen=maxlen)
    valid = data_gen.prepare_data(valid[0], valid[1], maxlen=maxlen)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
            m = RNNModel(is_training=True, config=config)
        with tf.variable_scope("model", reuse=True, initializer=initializer):
            mvalid = RNNModel(is_training=False, config=config)
